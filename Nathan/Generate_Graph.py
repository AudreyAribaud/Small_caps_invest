...
def _normalize_to_start(series: pd.Series, start_value: float) -> pd.Series:
    ...
    if not isinstance(series, (pd.Series, pd.Index, list, tuple, pd.array, pd.core.indexes.base.Index)):
        series = pd.Series([series])  # encapsule un scalaire dans une série
...
def download_sp500(dates: pd.Series, starting_equity: float = 100.0) -> pd.DataFrame:
    ...
    # Sécuriser : si aligned est DataFrame -> prendre la première colonne
    if isinstance(aligned, pd.DataFrame):
        aligned = aligned.iloc[:, 0]

    # Sécuriser : si aligned est un scalaire -> transformer en Series d’une seule valeur
    if not isinstance(aligned, pd.Series):
        aligned = pd.Series([aligned], index=pd.to_datetime(dates))
